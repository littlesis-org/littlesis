...
require_once(dirname(__FILE__).'./../../plugins/sfDoctrinePlugin/lib/task/sfDoctrineBaseTask.class.php');
 ...
 */
class BuildAllReloadMainTask extends sfDoctrineBaseTask
{
  /**
   * @see sfTask
   */
  protected function configure()
  {
    ...
    $this->aliases = array('doctrine-build-all-reload-main');
    $this->namespace = 'doctrine';
    $this->name = 'build-all-reload-main';
    $this->briefDescription = 'Generates Doctrine model, SQL, initializes database, and load data';

    $this->detailedDescription = <<<EOF
The [doctrine:build-all-reload-main|INFO] task is a shortcut for four other tasks:

  [./symfony doctrine:build-all-reload-main frontend|INFO]

The task is equivalent to:
  
  [./symfony doctrine:drop-main-db|INFO]
...
EOF;
  ...
  {
    $dropDb = new DropMainDbTask($this->dispatcher, $this->formatter);
