 ...
 */
function javascript_path($source, $absolute = false, $override = false)
{
  // CHANGE TO SYMFONY'S ASSETHELPER
  if (!$override && sfConfig::get('app_amazon_enable_s3_assets'))
  {
    $path = _compute_public_path($source, 'js', 'js', $absolute);

    $request = sfContext::getInstance()->getRequest();
    $sf_relative_url_root = $request->getRelativeUrlRoot();

    $s3_base = sfConfig::get('app_amazon_s3_base') . DIRECTORY_SEPARATOR .
      sfConfig::get('app_amazon_s3_bucket');

    if ($absolute)
    {
      $sf_base = 'http'.($request->isSecure() ? 's' : '').'://'.$request->getHost() . $sf_relative_url_root;
      $s3_path = substr($path, strlen($sf_base));      
    }
    else
    {
      $sf_base = $sf_relative_url_root;
      $s3_path = substr($path, strlen($sf_base));      
    }

    return $s3_base . $s3_path;
  }
 ...
 * @see    stylesheet_tag  
 */
function stylesheet_path($source, $absolute = false, $override = false)
{
  // CHANGE TO SYMFONY'S ASSETHELPER
  if (!$override && sfConfig::get('app_amazon_enable_s3_assets'))
  {
    $path = _compute_public_path($source, 'css', 'css', $absolute);
 ...
 * @see    image_path 
 */
function image_tag($source, $options = array(), $override = false)
{
  // THIS IS THE ONE CHANGE TO SYMFONY'S ASSETHELPER
  ...
  {
    $options['src'] = image_path($source, $absolute, $override);
  }
  else
  {
    $options['src'] = $source;
