...
YShout.prototype = {
  ...
  hasFocus: true,
	...
	initForm: function() {
		...
		var postForm = 
			'<form id="ys-post-form"' + (this.prefs.inverse ? 'class="ys-inverse"' : '' ) + '><fieldset>' +
				'<input id="ys-input-nickname" value="' + this.prefs.defaultNickname + '" type="hidden" accesskey="N" maxlength="' + this.prefs.nicknameLength + '" class="ys-before-focus" />' +
        '<strong>' + this.prefs.publicName + '</strong>' +
    ...
    $('#ys-input-nickname').val(this.prefs.publicName);
  ...
  blink: function() {
    var self = this;
    document.title = (document.title == this.originalTitle) ? this.blinkingTitle : this.originalTitle;    
    this.timer = window.setTimeout(function() { self.blink(); }, 2000);
  },
	
	startBlinking: function() {
    var self = this;
    window.setTimeout(function() { self.blink(); }, 1000);
	},
  ...
  initFocus: function() {
	...
	updateStatus: function(updates, init) {
    if (!init) {
      if (updates.posts) {      
        if (!this.hasFocus) {
          this.startBlinking();
