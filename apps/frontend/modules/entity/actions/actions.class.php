...
class entityActions extends sfActions
{
  ...
  public function executeMatchDonors($request)
  {
    ...
    $where = 'ed.reviewed_at IS NULL AND ed.locked_at IS NULL AND e.id <> ?';
    ...
    $sql = 'SELECT e.* FROM os_entity_donor ed LEFT JOIN entity e ON (ed.entity_id = e.id) WHERE ' . $where . ' LIMIT 100'; 
    ...
    //check for lock
    $sql = 'SELECT COUNT(*) FROM os_entity_donor WHERE entity_id = ? AND locked_by_user_id <> ? AND locked_by_user_id IS NOT NULL';
    ...
    //get preprocessed donor_ids
    $sql = 'SELECT donor_id FROM os_entity_donor WHERE entity_id = ? AND donor_id <> ?';
    $stmt = $db->execute($sql, array($this->entity['id'], '         '));
    $donorIds = $stmt->fetchAll(PDO::FETCH_COLUMN);


    //get verified donor_ids
    $sql = 'SELECT donor_id FROM os_entity_donor WHERE entity_id = ? AND is_verified = 1 AND donor_id <> ?';
    $stmt = $db->execute($sql, array($this->entity['id'], '         '));
    $this->verified_donor_ids = $stmt->fetchAll(PDO::FETCH_COLUMN);

    
    //verify and unverify donor ids
    if ($request->isMethod('post'))
    {
      ...
      if ($nextEntity && $commit == 'Skip and Match Another')
      {
        ...
        $this->redirect(EntityTable::getInternalUrl($nextEntity, 'matchDonors'));        
      }


      if (!$submittedDonorIds = $request->getParameter('donor_ids'))
      {
        $submittedDonorIds = array();
      }
      
      $newDonorIds = array_diff($submittedDonorIds, $this->verified_donor_ids);
      $oldDonorIds = array_diff($this->verified_donor_ids, $submittedDonorIds);

      //mark new donor ids as verified
      $sql = 'UPDATE os_entity_donor SET is_verified = 1, is_synced = (is_verified = is_processed), reviewed_at = ?, reviewed_by_user_id = ? WHERE entity_id = ? AND donor_id IN (' . implode(',', array_fill(0, count($newDonorIds), '?')) . ')';
      $params = array_merge(array(date('Y-m-d H:i:s'), $this->getUser()->getGuardUser()->id, $this->entity['id']), $newDonorIds);
      $stmt = $db->execute($sql, $params);
      
      //mark old donor ids as unverified
      $sql = 'UPDATE os_entity_donor SET is_verified = 0, is_synced = (is_verified = is_processed), reviewed_at = ?, reviewed_by_user_id = ? WHERE entity_id = ? AND donor_id IN (' . implode(',', array_fill(0, count($oldDonorIds), '?')) . ')';
      $params = array_merge(array(date('Y-m-d H:i:s'), $this->getUser()->getGuardUser()->id, $this->entity['id']), $oldDonorIds);
      $stmt = $db->execute($sql, $params);
      
      //mark all donor ids as reviewed
      $sql = 'UPDATE os_entity_donor SET reviewed_at = ?, reviewed_by_user_id = ? WHERE entity_id = ?';
    ...
    //lock page
    OsEntityDonorTable::lockEntityById($this->entity['id'], $userId);
    

    //get user who last reviewed, if any
    $this->reviewed_by_user = null;
    $this->reviewed_at = null;
    $sql = 'SELECT p.public_name, ed.reviewed_at FROM os_entity_donor ed ' .
           'LEFT JOIN sf_guard_user_profile p ON (ed.reviewed_by_user_id = p.user_id) ' .
           'WHERE ed.entity_id = ? LIMIT 1';
    $stmt = $db->execute($sql, array($this->entity['id']));
    ...
    if (count($donorIds))
    {
      //get all donation records for the given donor ids
      $rawDb = $databaseManager->getDatabase('raw');
      $rawDb = Doctrine_Manager::connection($rawDb->getParameter('dsn'));  
      $sql = 'SELECT * FROM os_donation WHERE donor_id IN (' . implode(',', array_fill(0, count($donorIds), '?')) . ')';
      $stmt = $rawDb->execute($sql, $donorIds);
    ...
    {
      if (isset($this->donors[$row['donor_id']]))
      {
        $this->donors[$row['donor_id']]['names'][$row['donor_name']] = true;
        $this->donors[$row['donor_id']]['cycles'][$row['cycle']] = true;
        $this->donors[$row['donor_id']]['orgs'][$row['employer_raw']] = true;
        $this->donors[$row['donor_id']]['addresses'][OsDonationTable::buildAddress($row)] = true;
        $this->donors[$row['donor_id']]['image_ids'][$row['fec_id']] = true;

        if ($row['donor_name_middle'])
        {
          $this->donors[$row['donor_id']]['middles'][$row['donor_name_middle']] = true;        
        }

      }
      else
      {
        $this->donors[$row['donor_id']] = array(
          ...
          'orgs' => array($row['employer_name'] => true),
          'addresses' => array(OsDonationTable::buildAddress($row) => true),
          'image_ids' => array($row['fec_id'] => true)
        );
      }
    }
