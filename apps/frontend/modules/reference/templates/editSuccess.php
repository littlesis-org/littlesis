...
<form action="<?php echo url_for('reference/edit') ?>" method="POST">
<?php echo input_hidden_tag('id', $reference->id) ?>
...
<table>
        ...
        <tr>
          <td>
            <?php foreach ($fields1 as $name => $label) : ?>
              <input type="checkbox" name="fields[<?php echo $name ?>]" value="1" <?php if (in_array($name, $reference->getFieldsArray())) { echo ' checked'; } ?>/> <?php echo $label ?><br />
            <?php endforeach; ?>
          </td>
  ...
  <tr>
    ...
    <td>
      ...
      <?php echo button_to('Delete', 'reference/remove?id=' . $reference->id, 'post=true confirm=Are you sure?') ?>
